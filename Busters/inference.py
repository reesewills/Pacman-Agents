        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    allPossible[p] += self.beliefs[p] * emissionModel[
                        trueDistance]  # P( X_t | X_t-1 ) * P( X_t-1 | E_1:t-1 )
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # take a random sampling of the legal positions of the ghosts initialized as a list
        self.particles = []
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            for idx in range(self.numParticles):
                self.particles[idx] = self.getJailPosition()
        else:
            updatedWeights = util.Counter()
            oldWeights = self.getBeliefDistribution()
            for particle in self.particles:
                dist_to_pacman = util.manhattanDistance(particle, pacmanPosition)
                updatedWeights[particle] = emissionModel[dist_to_pacman]

            #updatedWeights.normalize()

            if not any(updatedWeights.values()):
                self.initializeUniformly(gameState)
            else:
                updatedWeights.normalize()
                self.particles = []
                for idx in range(self.numParticles):
                    self.particles.append(util.sample(updatedWeights))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numParticles):
            # start our array here
            thisParticle = []

            for j in range(self.numGhosts):
                """
        remainingLegal = self.legalPositions
        for choice in thisParticle:
          if choice in remainingLegal:
            remainingLegal.remove(choice)
          if(remainingLegal):
            thisChoice = random.choice(remainingLegal)
            thisParticle.append(thisChoice)
        """
                # have we found a legal choice not in this particle yet?
                stillLooking = True
                while (stillLooking):
                    thisChoice = random.choice(self.legalPositions)
                    if thisChoice in thisParticle:
                        continue
                    else:
                        thisParticle.append(thisChoice)
                        stillLooking = False
            self.particles.append(tuple(thisParticle))
...
"""
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                positionDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i,
                                                               self.ghostAgents[i])
                newParticle[i] = util.sample(positionDist)
        ...
        for i in range(self.numParticles):
            ...
            for j in range(self.numGhosts):
                ...
                if (noisyDistances[j] != None):
                    manDist = util.manhattanDistance(thisParticle[j], pacmanPosition)
